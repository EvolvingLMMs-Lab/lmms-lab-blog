...
const PAGE_PATH = '/example-post';
const renderToc = (html) => renderTableOfContents(html, PAGE_PATH);
...
test('renderTableOfContents builds hierarchy and excludes heading controls', () => {
  const result = renderToc(`
    ...
    <h3>Derivation <span data-toc-ignore>Draft</span></h3>
    <h2>Examples</h2>
  `);
...
test('renderTableOfContents requires a root-relative page path', () => {
  assert.throws(
    () => renderTableOfContents('<h2>Section</h2>', ''),
    /pagePath must be an absolute path/,
  );
});
