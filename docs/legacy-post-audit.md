...
| `onevision-encoder` | `public/onevision-encoder/index.html`, `/onevision-encoder` | Hypothesis/method/evidence text, codec and uniform patch exhibits, seven-case processing pipeline, global-contrastive video, all reported LMM/attentive/patch-efficiency results, resource links, lab authorship, and BibTeX. | Kept as `index.html`, but only article body content enters Angular. Patch grids and pipeline controls are scoped local components; remote document dependencies, duplicate navigation, and inline scripts are not retained. Tables are normalized into accessible shared table markup without changing reported values. |
...
and 256 codec patch files while reusing sparse image nodes during animation.
