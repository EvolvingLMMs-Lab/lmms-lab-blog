...
const ACTIVATION_LOOKAHEAD_PX = 64;
...
export function flattenToc(items: readonly TocItem[]): TocItem[] {
...
export class HeadingObserver {
  ...
  observe(article: HTMLElement, tocItems: readonly TocItem[], hashId: string | null): void {
    this.disconnect();
  ...
  private sync(): void {
    ...
    const atPageEnd =
      ...
      Math.ceil(window.scrollY + window.innerHeight) >= documentHeight - 2;
  ...
  private getActivationLine(): number {
    ...
    return (Number.isFinite(scrollMargin) ? scrollMargin : 0) + ACTIVATION_LOOKAHEAD_PX;
  }
