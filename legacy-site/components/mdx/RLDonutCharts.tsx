...
function polarToCartesian(cx: number, cy: number, r: number, angleDeg: number) {
  ...
  // Node and Chromium can differ in the last few floating-point digits for
  // trigonometric functions. Stable coordinates keep the SVG identical during
  // server rendering and hydration without changing its rendered geometry.
  return {
    x: Number((cx + r * Math.cos(rad)).toFixed(10)),
    y: Number((cy + r * Math.sin(rad)).toFixed(10)),
  };
}
